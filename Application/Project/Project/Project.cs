...
namespace SKC
{
    ...
    public class Project
    {
        ...
        protected bool isselect;                    // Признак того, что данный проект выбран для действий;
        protected bool ismodify;                    // Признак того, что данный проект редактировался;
        protected int internalNmb;                  // Внутренний номер, используется для поддержания исходного порядка
        ...
        /// <summary>
        /// Определяет время завершения работы
        /// </summary>
        ...
        public class SortByName : IComparer<Project>
        {
            public int Compare(Project p1, Project p2)
            {
                int res = string.Compare(p1.place, p2.place);
                if (res != 0) return res;
                res = string.Compare(p1.bush, p2.bush);
                if (res != 0) return res;
                res = string.Compare(p1.well, p2.well);
                if (res != 0) return res;
                return string.Compare(p1.job, p2.job);
            }
        }

        public class SortByNmb : IComparer<Project>
        {
            public int Compare(Project p1, Project p2)
            {
                int res = p1.internalNmb - p2.internalNmb;
                return res;
            }
        }
    }
